...
enum vx_graph_attribute_internal_type_e
{
    ...
    /* network binary graph name */
    VX_GRAPH_SET_NETWORK_BINARY_GRAPH_NAME        = VX_ATTRIBUTE_BASE(VX_ID_VIVANTE, VX_TYPE_GRAPH) + 0xD,
};
...
enum vx_nn_activation_function_e
{
    ...
    VX_NN_ACTIVATION_PRELU = VX_ENUM_BASE(VX_ID_VIVANTE, VX_ENUM_NN_ACTIVATION_FUNCTION_TYPE) + 0x1f,
};
...
VX_API_ENTRY vx_tensor VX_API_CALL vxCreateTensorFromAXI(vx_context context, vx_graph graph, const vx_tensor_create_params_t* tensor_create_params, vx_size size_of_create_params);
...
typedef struct _vx_hardware_caps_params_t
{
    ...
    vx_uint32 coreCount;      /*!< \brief  hardware core count.*/
#endif
...
VX_API_ENTRY vx_node VX_API_CALL vxInstanceNormalizationSplitLayer0(
    vx_graph                    graph,
    vx_tensor                   input,
...
VX_API_ENTRY vx_node VX_API_CALL vxInstanceNormalizationSplitLayer1(
    vx_graph                    graph,
    vx_tensor*                  input_list,
    vx_uint32                   input_count,
    vx_tensor*                  output_list,
...
VX_API_ENTRY vx_node VX_API_CALL vxInstanceNormalizationSplitLayer2(
    vx_graph                    graph,
    vx_float32                  eps,
    vx_uint64                   area,
    vx_tensor*                  input_list,
...
#if VX_GRAPH_BATCH_OPT_SUPPORT
/*! \brief set node splitting on batch dimension.
* \param [in] node The node reference <tt>\ref vx_node</tt>.
* \param [in] batch_split_num The number of splits.
* \return A <tt>vx_status_e</tt> enumeration.
* \retval VX_SUCCESS No errors.
* \retval VX_SUCCESS Success
* \retval * An error occurred. See <tt>vx_status_e</tt>.
*/
VX_API_ENTRY vx_status VX_API_CALL vxSetNodeBatch(
    vx_node node,
    vx_uint32 batch_split_num
);
#endif

/*! \brief [Graph] Creates a layer Normalization Node.
 * \details The root mean square of a set of numbers is the quare root of the sum of the squares of the numbers divided by the number of terms.
 * \param [in] graph The handle to the graph.
 * \param [in] eps [static] Float 32. Small value to add to the variance estimate so that we don't divide by zero.(default is 1e-8)
 * \param [in] axis [static] The axis on which we need do normalize.
 * \param [in] input_list [static] The input tensor data.
 * \param [in] input_count [static] The input tensor number.
 * \param [out] output [static] The output tensor data.
 ...
 * \ingroup group_cnn
 */
VX_API_ENTRY vx_node VX_API_CALL vxRMSNormalizationLayer(
    vx_graph                    graph,
    vx_float32                  eps,
    vx_int32                    axis,
    vx_tensor*                  input_list,
...
/*! \brief [Graph] Creates a layer logsoftmax activation Node.
 * \param [in] graph The handle to the graph.
 * \param [in] beta [static] Float32. The parameter in the logsoftmax equation.
 * \param [in] axis [static] The axis on which we need to do logsoftmax.
 * \param [in] input The input tensor </tt>.
 * \param [out] output The output tensor data, Output will have the same number of dimension ass input tensor.
 * \return <tt> vx_node</tt>.
 * \returns A node reference <tt>\ref vx_node</tt>. Any possible errors preventing a
 * successful creation should be checked using <tt>\ref vxGetStatus</tt>.
 * \ingroup group_cnn
 */
VX_API_ENTRY vx_node VX_API_CALL vxLogSoftMaxLayer(
    vx_graph                    graph,
    vx_tensor                   input,
    vx_float32                  beta,
    vx_int32                    axis,
